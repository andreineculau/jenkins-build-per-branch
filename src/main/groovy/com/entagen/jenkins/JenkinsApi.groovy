...
class JenkinsApi {
    ...
    String configForMissingJob(ConcreteJob missingJob, List<TemplateJob> templateJobs) {
        ...
        config = config.replaceAll("(\\p{Alnum}*[>/])([^<]*${templateJob.templateBranchName}[^<]*)<") { fullMatch, prefix, configValue ->
            ...
            if (ignoreTags.find { it + ">" == prefix}) {
              ...
              configValue = configValue.replace(templateJob.templateBranchName, missingJob.branchName)
